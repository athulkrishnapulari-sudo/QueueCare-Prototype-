...
const signinFormCard = document.getElementById('signinFormCard');
      ...
      authSwitches.forEach((link) => {
        link.addEventListener('click', (e) => {
          ...
          window.location.hash = target === 'register' ? 'registerFormCard' : 'signinFormCard';
          setActiveForm(target);
        });
      });

      window.addEventListener('hashchange', () => {
        const target = window.location.hash === '#registerFormCard' ? 'register' : 'signin';
        setActiveForm(target);
      });
      ...
      // Initial state
      const initialTarget = window.location.hash === '#registerFormCard' ? 'register' : 'signin';
      setActiveForm(initialTarget);
const registerForm = document.getElementById('registerForm')

ScriptUrl="https://script.google.com/macros/s/AKfycbyEYPZlZ7-S-CoRDLjb2NCOesBX-1f9Wxs-LuQ8_qNJnKKFumDEAQuvu5wSf0CyKtbNHA/exec"
...
registerForm.addEventListener("submit", (e) => {
    ...
    .then(result => {
        console.log(result);
        alert("Account Created Successfully. Sign In Your Account");
        registerForm.reset();
        setActiveForm("signin");
        window.location.hash = "signinFormCard";
